...
def center_triplet(x, targets, weights, neg_r, norm=True, end2end=False):
    N = x.shape[0]
    M = weights.shape[0]
    weights_target = torch.Tensor([i for i in range(M)]).long().cuda()
    if norm:
       x = F.normalize(x, dim=1)
       weights = F.normalize(weights, dim=1)
    if end2end is not True:
       weights = weights.detach()
    
    is_pos = targets.view(N, 1).expand(N, M).eq(weights_target.view(1, M).expand(N, M))
    is_neg = targets.view(N, 1).expand(N, M).ne(weights_target.view(1, M).expand(N, M))
    
    sim_mat = torch.mm(x, weights.t()) 
    sim_ap = sim_mat[is_pos].contiguous()
    sim_an_mat = sim_mat[is_neg].contiguous().view(N, -1)
    sim_an = torch.logsumexp(sim_an_mat*neg_r, 1)/neg_r 
    loss = torch.log(1.0+torch.exp(sim_an-sim_ap)).mean()
...
class EmbeddingHead(nn.Module):
    def __init__(self, cfg):
        ...
        self.neg_r = nn.Parameter(torch.ones(1) * 50.0)
    ...
    def forward(self, features, targets=None):
        ...
        # Training
        center_tri_loss = None
        if self.classifier.__class__.__name__ == 'Linear':
            cls_outputs = self.classifier(bn_feat)
            pred_class_logits = F.linear(bn_feat, self.classifier.weight)
            # hardtriplet,   logsumexp
            center_tri_loss = center_triplet(bn_feat, targets, self.classifier.weight, 30.0, norm=True, end2end=False)
        ...
        return {
            "cls_outputs": (cls_outputs, center_tri_loss),
